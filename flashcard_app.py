...
class OptionsDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        self.font_size.setValue(int(self.settings.get("font_size", 12)))
...
class FlashcardApp(QWidget):
    ...
    def build_ui(self):
        ...
        # Table - Note: Column order changed - Delete is now column 5
    ...
    def load_data(self):
        ...
        if text:
            query += " AND (lesson LIKE ? OR front LIKE ? OR back LIKE ?)"
            params += [f"%{text}%"] * 3

        elif self.filter_selected:  # Only apply selected filter if no search text
            query += " AND selected = 1"

        query += " ORDER BY lesson"  # Always sort by lesson
    ...
    def add_row(self, row):
        ...
        delete_btn.clicked.connect(lambda checked, card_id=id_: self.delete_row(card_id))
        self.table.setCellWidget(row_idx, 5, delete_btn)

        # Column 6: Printed count
        printed_count = QTableWidgetItem(str(printed))
        printed_count.setTextAlignment(Qt.AlignCenter)
        self.table.setItem(row_idx, 6, printed_count)
        
        # Column 7: Last printed
        self.table.setItem(row_idx, 7, QTableWidgetItem(last or ""))
